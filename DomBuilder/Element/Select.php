...
class Select extends \DomBuilder\Core\Element\Field
{
  /** 
   * Constructor.
   */
   ...
   *
   * @param array $option key of array is value of value attribute for option element and 
   *                      value of array is value of option element.
  ...
  public function option($option)
  {
    ...
    $this->_option = $option;
    foreach($this->_option as $k=>$v) $this->insert('option')->value($k)->html($v);
    return $this;
  }
  ...
  protected function _element()
  {
    ...
    if( $value !== false ) $this->_option($value);
    $text = $this->_text();
    $html = $this->_tagOpen();
   ...
   * Sets field value.   
   */     
  private function _option($value)
  {
